...
class SpriteGroup:
    ...
    def writeSpritesToFree(self, rom):
        ...
        for spf in self._sprites:
            ...
            try:
                ...
                try:
                    ...
                    spritePtrs.append((uniqueSprites.index(sp), True))
        # Find a free block
        loc = rom.allocate(size=sum(map(lambda x: x.blockSize(), uniqueSprites)),
                           can_write_to=(lambda offset: (offset & 15) == 0))
        ...
        self._spritePtrs = map(
            lambda n_f: (locStart + n_f[0] * spBlockSize) | n_f[1],
            spritePtrs)
